# Shadow Chat — Gesamtprojekt (zusammengeführt)

Dieser Ordner ist die **eine, lauffähige Zusammenführung** aller Ordner aus dem
Repository `Shadow3` (`Shadow Chat — Quellcode-Archiv`, `Shadow/shadow-mvp`,
`shadow-mvp`, `Zips/…`). Bei doppelten Dateien wurde jeweils die **neueste und
vollständigste Version** übernommen; ältere Teilkopien, `.DS_Store`,
`__pycache__` und Test-Datenbanken wurden entfernt.

## Schnellstart

```bash
./start.sh              # baut den Rust-Server und startet Server + Web-UI
./start.sh --prebuilt   # ohne Rust: fertige Linux-Binary aus prebuilt/ nutzen
```

- API:    http://127.0.0.1:8787
- Web-UI: http://127.0.0.1:8080
- Erster Login: `Admin` / `1234` (danach wird ein neuer Admin erzwungen)

Nur Server bzw. CLI:

```bash
cargo build --release
./target/release/shadow server --host 127.0.0.1 --port 8787 --data-dir ./shadow-data
./target/release/shadow login | chat | users | models | bench
```

LLM-Engine anbinden (sonst läuft der Stub-Adapter):

```bash
export SHADOW_AI_CMD="python3 -m shadow_ai"   # Modul in shadow-ai/
```
...
| Pfad | Inhalt |
|---|---|
| `Cargo.toml`, `crates/` | Rust-Workspace: `shadow-core`, `shadow-server` (axum-API), `shadow-cli` |
| `flutter_ui/` | Flutter-UI (Quellcode, Web + Desktop-Scaffolds) |
| `shadow-ai/` | Python-Anbindung der Shadow LLM Engine (Stub/Echo-Modell) |
| `prebuilt/linux-x64/` | Fertiger Linux-Server + Flutter-Desktop-UI |
| `prebuilt/web-ui/web/` | Fertiger Flutter-Web-Build (kein Flutter SDK nötig) |
| `prebuilt/windows-x64/` | `shadow.exe` + Startskript für Windows |
| `docs/ARCHITEKTUR.md` | Architektur- und Feature-Dokumentation |
| `UPDATE.md` | Änderungsverlauf Auth/Chat/Admin (Schema v1 → v2) |

## Verifiziert in diesem Zustand

- `cargo build --release` erfolgreich (nur 2 harmlose Warnungen: unbenutzter Import / unbenutzte Funktion)
- `cargo test --release`: 10 Tests grün, 2 ignoriert
- Server-Smoke-Test: `/api/auth/status`, Login `Admin/1234`, `/api/models`,
  Session anlegen und Nachricht senden (Stub-Adapter antwortet)
